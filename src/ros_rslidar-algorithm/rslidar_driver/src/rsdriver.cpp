...
rslidarDriver::rslidarDriver(ros::NodeHandle node, ros::NodeHandle private_nh)
{
  ...
  private_nh.param("frame_id", config_.frame_id, std::string("livox_frame"));
  ROS_DEBUG_STREAM("config_.frame_id: " << config_.frame_id);
  ...
  int npackets = (int)ceil(packet_rate / frequency);  //雷达转一圈多少个packet
  private_nh.param("npackets", config_.npackets, npackets);
  ROS_INFO_STREAM("[driver] publishing " << config_.npackets << " packets per scan");   //转一圈 == per scan
...
{  // Allocate a new shared pointer for zero-copy sharing with other nodelets.
  ...
  else  // standard behaviour
  {
    if (difop_input_->getUpdateFlag())
    {
      ...
      else if ((config_.model == "RS32" || config_.model == "RSBPEARL" || config_.model == "RSBPEARL_MINI") &&
               (mode == 0))
      {

...
#include "attitude.h"
extern Skywell::Attitude attitude_truck;
namespace rslidar_rawdata
{
const float FRONT_WHEEL_DISTANCE = 0.31f;          /*前轮距*/
const float LEFT_LIDAR_TO_WHEEL_DISTANCE = 2.51f;  /*左雷达距左前轮的距离*/
const float RIGHT_LIDAR_TO_WHEEL_DISTANCE = 0.31f; /*右侧雷达距右前轮的距离*/
...
typedef Eigen ::Matrix<float, 3, 1> Vect_3f;
typedef Eigen ::Matrix<float, 3, 3> Matrix_33f;
...
private:
  ...
  Eigen::Vector3f euler_sta_angle_;
  Eigen::Vector3f atti_coor_;
